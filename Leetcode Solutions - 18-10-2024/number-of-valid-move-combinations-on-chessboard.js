class CircularQueue {
  constructor(capacity) {
    ...
    this._size = 0
    this._bottom = 0
    this._data = new Array(capacity).fill(undefined)
  }
...
function encode(data, n) {
  let res = 0
  for (let i = n - 1; i >= 0; i--) {
    res = (res << 7) | (data.steps[i] << 4) | (data.dir[i] << 1) | data.stopped[i]
  }
  return res
}
...
const countCombinations = function (pieces, positions) {
  ...
  function btNext(i, codePre) {
    ...
    for (let j = 0; j < i; j++) {
      if (dataCur.c[i] == dataCur.c[j] && dataCur.r[i] == dataCur.r[j]) return
    }
    btNext(i + 1, codePre)
  }
  let maxSize = q.size
