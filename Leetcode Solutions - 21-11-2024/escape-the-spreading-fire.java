    ...
    public boolean ok(int[][] grid, int[][] dist, int wait_time) {
        ...
        while (!Q.isEmpty()) {
            ...
            for (int[] to : moves) {
                ...
                if (!inBounds(ii, jj, n, m) || visited[ii][jj] == 1 || grid[ii][jj] == 2)
                    continue;
                if (ii == n - 1 && jj == m - 1 && dist[ii][jj] >= at.third + 1)
                    return true;
                if (dist[ii][jj] <= at.third + 1)
                    continue;
    ...
    public int maximumMinutes(int[][] grid) {
        ...
        while (!Q.isEmpty()) {
            ...
            for (int[] to : moves) {
                int ii = at.first + to[0];
                int jj = at.second + to[1];
                if (!inBounds(ii, jj, n, m) || grid[ii][jj] == 2 || dist[ii][jj] <= at.third + 1)
                    continue;
